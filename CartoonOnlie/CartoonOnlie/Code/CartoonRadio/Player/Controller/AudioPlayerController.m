...
@interface AudioPlayerController () <UMSocialUIDelegate>
{
...
- (void)updateAudioPlayer{
    ...
    NSLog(@"%@", model.url);
    playerItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:model.url]];
...
- (void)updateUIDataWith:(MusicModel *)model{
    ...
    self.singerLabel.text = model.sub_title;
...
- (void)setLockViewWith:(MusicModel*)model currentTime:(CGFloat)currentTime
{
    ...
    // 设置Singer
    [musicInfo setObject:model.sub_title forKey:MPMediaItemPropertyArtist];
    // 设置歌曲名
    [musicInfo setObject:model.sub_title forKey:MPMediaItemPropertyTitle];
    // 设置封面
